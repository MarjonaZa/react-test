...
class WomenViewSet(viewsets.ModelViewSet):
    ...
    @action(methods=['get'], detail=True) #  с помощью декоратора добавляем всякие разные маршруты
...
#     queryset = Women.objects.all() #как-будто бы все данные но нет, с помощью UpdateAPIView метод queryset будет барть только 1 элемент
#     serializer_class = WomenSerializer
#
# class WomenAPIAll(generics.RetrieveUpdateDestroyAPIView):
